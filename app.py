...
# Lista de fotos que será compartilhada entre as sessões (simulado via session_state)
...
if not st.session_state.usuario_logado:
    ...
    if st.button("Começar Avaliação"):
        if nome and foto_perfil:
            ...
            # Adiciona a foto do novo usuário à lista para que outros possam vê-la
            # Nota: Em produção, isso exigiria um banco de dados para ser global.
            st.session_state.fotos_cadastradas.append(foto_perfil)
            st.rerun()
        elif not nome:
    ...
    with col_perfil2:
        st.write(f"Olá, **{st.session_state.nome_usuario}**! Deslize ou clique nas fotos de outros usuários:")
    
    # Seleciona a imagem da lista (incluindo as enviadas por usuários)
    total_fotos = len(st.session_state.fotos_cadastradas)
    imagem_atual = st.session_state.fotos_cadastradas[st.session_state.indice_imagem % total_fotos]
    
    # Se for uma URL (string), usa o componente de Swipe. 
    # Se for um objeto de upload (bytes), exibe via st.image (o componente HTML nativo não lê bytes locais diretamente sem processamento extra)
    if isinstance(imagem_atual, str):
        swipe_js = f"""
        <style>
            .container {{
                position: relative;
        ...
        # Exibição de fotos enviadas por outros usuários (objetos de imagem)
        st.image(imagem_atual, width=400)
